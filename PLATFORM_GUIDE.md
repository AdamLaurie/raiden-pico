...
#### Integrated crowbar gate via EXTERNAL power mode (GP11)

Instead of dedicating a spare pin to the MOSFET gate, you can re-task the target
power group: `TARGET POWER MODE EXT` turns GP10 into a supply enable and **GP11 into a
PIO-driven crowbar gate** that emits the same glitch waveform as GP2 (same WIDTH/GAP/COUNT,
any trigger type — generated by a second PIO state machine fired from the same trigger),
leaving GP12 as a reserved spare. This keeps GP2 free as the polarity-aware
trigger for an external glitcher and avoids spending another GPIO.

```
TARGET POWER MODE EXT          # GP11 = crowbar gate, active-HIGH/idle-LOW (default)
TARGET POWER MODE EXT ALOW     # ...or active-LOW/idle-HIGH for a high-side P-FET
SET WIDTH 3                    # crowbar conduction window (cycles × 6.67 ns)
ARM ON                         # GP11 now emits the glitch waveform on every trigger
```

Wiring is the same MOSFET circuit as above, but the gate connects to **GP11** instead of
GP15. The gate idles de-asserted at boot, while disarmed, and on disarm, so it can't clamp
the rail by accident. Disarm (`ARM OFF`) before switching back with `TARGET POWER MODE INT`.
